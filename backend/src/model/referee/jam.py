...
from model.jam import Jam, JamId, Score, StopReason, TeamString
...
class JamReferee(AbstractReferee):
    ...
    def get_jam(self, jam_id: JamId) -> Jam:
        ...
        return self.context.jams[period_num][jam_num]

    def get_lens(self) -> tuple[int, int]:
        return (len(self.context.jams[0]), len(self.context.jams[1]))

    def get_latest_jam_id(self) -> JamId:
        period_num: int = 1 if len(self.context.jams[1]) > 0 else 0
        jam_num: int = len(self.context.jams[period_num]) - 1
    ...
    def get_total_score(self, team: TeamString) -> int:
        all_jams: list[Jam] = [jam for period in self.context.jams for jam in period]
    ...
    def stop_jam(self, timestamp: datetime) -> None:
        ...
        elif jam.elapsed >= self.context.JAM_DURATION:
        ...
        self.context.jams[period_num].append(Jam())
